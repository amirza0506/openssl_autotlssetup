                ...
                int port)
{
    ...
    if (!group)
        group = "X25519MLKEM768:X25519:prime256v1:secp384r1";

    if (!SSL_CTX_set_cipher_list(ctx, cipher)) {
        printf("Cannot set cipher list\n");
        ERR_print_errors_fp(stderr);
    }

    if (!SSL_CTX_set_ciphersuites(ctx, ciphersuite)) {
        printf("Cannot set ciphersuite list\n");
        ERR_print_errors_fp(stderr);
    }

    if (!SSL_CTX_set1_groups_list(ctx, group)) {
        printf("Cannot set group list\n");
        ERR_print_errors_fp(stderr);
    }
    ...
    printf("TLS Version Forced : %s\n", tlsver ? tlsver : "AUTO");
    printf("Cipher List        : %s\n", cipher);
    printf("Ciphersuite List   : %s\n", ciphersuite);
    printf("Groups             : %s\n", group);
