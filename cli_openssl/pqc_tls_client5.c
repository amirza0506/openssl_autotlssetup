...
int main(int argc, char **argv) {
    ...
    if (c == 1) {
        ...
        if (insecure) printf("Client running in INSECURE mode (no server cert verification).\n");
        else printf("Client will verify server cert using ./certs/ca.crt\n");
        do_connect(insecure, groups, host);
    } else {
