...
void broadcast_message(char *msg, int len, SSL *sender)
{
    pthread_mutex_lock(&clients_mutex);
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i]) {
            SSL_write(clients[i], msg, len);
        }
    }
    pthread_mutex_unlock(&clients_mutex);
}
...
void *server_input(void *arg)
{
    ...
    while (fgets(msg, sizeof(msg), stdin)) {
        ...
        snprintf(formatted, sizeof(formatted), "[SERVER]: %s", msg);

        broadcast_message(formatted, strlen(formatted), NULL);
    }
...
void *client_handler(void *arg)
{
    ...
    while (1)
    {
        ...
        snprintf(msg, sizeof(msg), "[Client %p]: %s", ssl, buffer);

        printf("%s", msg);

        broadcast_message(msg, strlen(msg), ssl);
    }
