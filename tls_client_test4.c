...
int main()
{
    ...
    printf("TLS version (SSL3/TLS1.0/TLS1.1/TLS1.2/TLS1.3 or blank): ");
    fgets(input, sizeof(input), stdin);
    input[strcspn(input, "\n")] = 0;
    if (strlen(input) > 0) tls_version = strdup(input);

    printf("Cipher (TLS <=1.2 or blank): ");
    fgets(input, sizeof(input), stdin);
    input[strcspn(input, "\n")] = 0;
    if (strlen(input) > 0) cipher = strdup(input);

    printf("Ciphersuite (TLS 1.3 or blank): ");
    fgets(input, sizeof(input), stdin);
    input[strcspn(input, "\n")] = 0;
    if (strlen(input) > 0) ciphersuite = strdup(input);
